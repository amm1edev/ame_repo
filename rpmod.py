    ...
    async def dobrpcmd(self, message):
        ...
        try:
            ...
            if len(lenght_args) >= 3:
                ...
                if len(lst) == 0:
                    await message.edit(
                        "<b>В 3 секции были введены не эмодзи. Если были введены"
                        " эмодзи, но всё равно выходит ошибка, обратись к:"
                        " </b>@trololo_1"
                    )
                    return
                ...
                if not key_rp or not key_rp.strip():
                    ...
                    if not value_rp or not value_rp.strip():
                        ...
                        if int(len(key_len)) > 1:
                            await message.edit(
                                "<b>В качестве рп команды было введено больше одного"
                                " слова.</b>"
                            )
                        else:
                            if key_rp == "all":
                                await message.edit(
                                    "<b>Использовать '<code>all</code>' в качестве"
                                    " названия команды запрещено!</b>"
                                )
                            else:
                                if count_emoji == 1:
                                    ...
                                    await message.edit(
                                        f"<b>Команда '<code>{key_rp}</code>' успешно"
                                        f" добавлена с эмодзи '{emoji_rp}'!</b>"
                                    )
                                else:
                                    dict_rp_copy[key_rp] = value_rp
                                    self.db.set("RPMod", "rpcomands", dict_rp_copy)
    ...
    async def rpnickcmd(self, message):
        ...
        if len(lst) > 3:
            ...
            if len(lst) + len(nick) >= 45:
                await message.edit(
                    "<b>Ник превышает лимит в 45 символов(возможно эмодзи имеют длину"
                    " более 1 символа).</b>"
                )
            else:
            ...
            try:
                ...
                if len(lenght_args) >= 3:
                    ...
                    if not dict == type(emj):
                        await message.edit(
                            "<b>Синтаксис секции эмодзи не является корректным(словарём"
                            " в питоне).</b>"
                        )
                    else:
                        if len(emj) == 0:
                        ...
                        for x in emj.values():
                            ...
                            if not x or not x.strip():
                                await message.edit(
                                    "<b>Пустое значение в словаре для эмодзи? Да ты"
                                    " гений.</b>"
                                )
                                return
                            if (
                                ...
                                await message.edit(
                                    "<b>Были введены не только эмодзи в словаре для"
                                    " эмодзи(пробел тоже символ). </b>"
                                )
                                return
                            if len(lst) > 3:
                                await message.edit(
                                    "<b>Было введено более 3 эмодзи в словаре для"
                                    " эмодзи. </b>"
                                )
                                return
                        ...
                        for x in emj:
                            ...
                            if int(len(key_len)) > 1:
                                await message.edit(
                                    "<b>В качестве ключа было введено больше одного"
                                    " слова в словаре эмодзи.</b>"
                                )
                                return
                            if not x or not x.strip():
                                await message.edit(
                                    "<b>Пустой ключ в словаре для эмодзи? Да ты"
                                    " умничка.</b>"
                                )
                                return
                com = eval(comands)
                if dict == type(com):
                    ...
                    for x in com:
                        ...
                        if int(len(key_len_cmd)) > 1:
                            await message.edit(
                                "<b>В качестве ключа было введено больше одного слова в"
                                " словаре команд.</b>"
                            )
                            return
                        if not x or not x.strip():
                            await message.edit(
                                "<b>Пустой ключ в словаре для команд? Мой хороший, так"
                                " дела не делаются.</b>"
                            )
                            return
                    for x in com.values():
                        if not x or not x.strip():
                            await message.edit(
                                "<b>Пустое значение в словаре для команд? Не сегодня,"
                                " мой золотой.</b>"
                            )
                            return
                    if count_emoji == 1:
                        ...
                        await message.edit(
                            "<b>Успешное обновлени словаря команд и эмодзи! Вы можете"
                            " просмотреть их благодаря команде"
                            " '<code>.rplist</code>'.</b>"
                        )
                    else:
                        comand_copy = comand.copy()
                        ...
                        self.db.set("RPMod", "rpcomands", merge_com)
                        await message.edit(
                            "<b>Успешное обновлени словаря команд! Вы можете"
                            " просмотреть его благодаря команде"
                            " '<code>.rplist</code>'.</b>"
                        )
                else:
                    await message.edit(
                        "<b>Синтаксис секции команд не является корректным(словарём в"
                        " питоне).</b>"
                    )
            except:
                await message.edit(
                    "<b>Что то не так с разделителями /.\nЛибо не корректный"
                    " словарь.(либо вообще пусто)</b>"
                )
        ...
        if not args:
            ...
            if a.id in ex_copy:
                ...
                try:
                    ...
                    name = a.first_name
                await message.edit(
                    f"<i>Чат <b><u>{name}</u></b>(<code>{a.id}</code>) удален из"
                    " исключений.</i>"
                )
            else:
                ex_copy.append(a.id)
                ...
                try:
                    ...
                    name = a.first_name
                await message.edit(
                    f"<i>Чат <b><u>{name}</u></b>(<code>{a.id}</code>) добавлен в"
                    " исключения.</i>"
                )
        elif args.isdigit():
            args = args.strip()
            ...
            if args in ex_copy:
                ...
                try:
                    ...
                    name = a.first_name
                await message.edit(
                    f"<i>Чат <b><u>{name}</u></b>(<code>{args}</code>) удален из"
                    " исключений.</i>"
                )
            else:
                            ...
                            if detail[0] == i:
                                if detail[0] in emojies.keys():
                                    if len(lines) < 2:
                                        if rezjim == 1:
                                            await message.edit(
                                                f"{emojies[detail[0]]} | <a"
                                                f" href=tg://user?id={me.id}>{nick}</a>"
                                                f" {comand[detail[0]]} <a"
                                    ...
                                    if len(lines) < 2:
                                        if rezjim == 1:
                                            await message.edit(
                                                f"<a href=tg://user?id={me.id}>{nick}</a>"
                                                f" {comand[detail[0]]} <a"
                                                f" href=tg://user?id={user.id}>{user.first_name}</a>"
                                                + detail[1]
                                            )
