...
class DaysToMyBirthday(loader.Module):
    ...
    strings = {
        "name": "BirthdayTime",
        "date_error": (
            "<emoji document_id=5422840512681877946>❗️</emoji> <b>В конфиге не указан"
            " день вашего рождения, пожалуйста, исправь это :)</b>"
        ),
        "msg": (
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                ...
                None,
                lambda: (
                    "Дата вашего рождения. Указывать в формате ДД.ММ. Пример: 17.06"
                ),
            )
        )
    ...
    async def btname(self, message):
        ...
        if self.db.get(__name__, "change_name"):
            ...
            await utils.answer(
                message,
                (
                    "<emoji document_id=6325696222313055607>😶</emoji>Хорошо, я больше"
                    " не буду изменять ваше имя"
                ),
            )
            ...
            await utils.answer(
                message,
                (
                    "<b><emoji document_id=6327560044845991305>😶</emoji> Хорошо, теперь"
                    " я буду изменять ваше имя в зависимости от количества дней до дня"
                    " рождения</b>"
                ),
            )
