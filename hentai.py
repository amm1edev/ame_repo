...
# ---------------------------------------------------------------------------------
# Name: hentai
# Description: 🍓 Hentai doujin module 18+
# Author: CakesTwix
# Commands:
# .nhrandom | .nhlast | .nhid | .nhsearch
# ---------------------------------------------------------------------------------
...
from NHentai import CloudFlareSettings, NHentaiAsync
...
class NHentaiMod(loader.Module):
    ...
    async def client_ready(self, client, db) -> None:
        self.nhentai_async = NHentaiAsync(
            request_settings=CloudFlareSettings(
                csrftoken=self.config["CONFIG_CSRFTOKEN"],
    ...
    async def nhrandomcmd(self, message):
        ...
        await message.client.send_file(
            message.chat_id, hentai.cover.src, caption=StringBuilder(hentai)
        )

    async def nhlastcmd(self, message):
        """⌚️ Latest hentai doujin"""
        hentai = await self.nhentai_async.get_doujin(
            (await self.nhentai_async.get_page(page=1)).doujins[0].id
        )
        await message.delete()
        await message.client.send_file(
            message.chat_id, hentai.cover.src, caption=StringBuilder(hentai)
            ...
            if len(hentai.doujins) != 1:
                markup.append(
                    [
                        {
                            "text": "➡️",
                            "callback": self.hentai_pagination__callback,
                            "args": (hentai.doujins, 0, "+"),
                        }
                    ]
                )
    ...
    async def hentai_pagination__callback(self, call, list_doujins, index, type_button):
        ...
        await call.edit(
            text=StringBuilder(list_doujins[index]),
            reply_markup=markup,
            photo=list_doujins[index].cover.src,
        )
