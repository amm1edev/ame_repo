...
# ---------------------------------------------------------------------------------
# Name: datamosh
# Description: DataMosh effect to video
# Author: KeyZenD
# Commands:
# .datamosh
# ---------------------------------------------------------------------------------
    ...
    async def datamoshcmd(self, message):
        ...
        if not reply:
            await message.edit(
                "".join(
                    [
                        random.choice(html).format(ch)
                        for ch in self.strings("reply", message)
                    ]
                )
            )
            return
        ...
        await message.edit(
            "".join(
                [
                    random.choice(html).format(ch)
                    for ch in self.strings("processing", message)
                ]
            )
        )
        subprocess.call(
            f"ffmpeg -loglevel quiet -y -i {fn}1.mp4 -crf 0 -bf 0 {fn}1.avi", shell=True
        )
        try:
            _f = open(fn + "1.avi", "rb")
            f_ = open(fn + "2.avi", "wb")
            ...
            await message.edit(
                "".join(
                    [
                        random.choice(html).format(ch)
                        for ch in self.strings("error", message)
                    ]
                )
            )
            os.system(f"rm -f {fn}*")
        ...
        await message.client.send_file(
            message.to_id, file=fn + "2.mp4", video_note=bool(reply.video_note)
        )
        os.system(f"rm -f {fn}*")
