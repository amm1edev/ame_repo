...
# ---------------------------------------------------------------------------------
# Name: userinfo
# Description: Tells you about people
# Author: HitaloSama
# Commands:
# .userinfo | .permalink
# ---------------------------------------------------------------------------------
...
class UserInfoMod(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            "ENCODE", False, lambda m: self.strings("encode_cfg_doc", m)
        )
    ...
    async def userinfocmd(self, message):
        ...
        logger.debug(full)
        reply = self.strings("first_name", message).format(
            self._handle_string(full.user.first_name)
        )
        if full.user.last_name is not None:
            reply += self.strings("last_name", message).format(
                self._handle_string(full.user.last_name)
            )
        ...
        reply += self.strings("restricted", message).format(
            utils.escape_html(str(full.user.restricted))
        )
        reply += self.strings("deleted", message).format(
            utils.escape_html(str(full.user.deleted))
        )
        reply += self.strings("bot", message).format(
            utils.escape_html(str(full.user.bot))
        )
        reply += self.strings("verified", message).format(
            utils.escape_html(str(full.user.verified))
        )
        if full.user.photo:
            reply += self.strings("dc_id", message).format(
                utils.escape_html(str(full.user.photo.dc_id))
            )
    ...
    async def permalinkcmd(self, message):
        ...
        if len(args) > 1:
            await utils.answer(
                message,
                self.strings("permalink_txt", message).format(uid=user.id, txt=args[1]),
            )
        else:
            await utils.answer(
                message, self.strings("permalink_uid", message).format(uid=user.id)
            )
