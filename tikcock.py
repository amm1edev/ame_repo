    ...
    async def ttdlcmd(self, message):
        ...
        if not args:
            ...
            async with httpx.AsyncClient() as client:
                if tik_info := re.findall(
                    r"\/.*\/([\d]*)?",
                    (
                        (await client.head(args)).headers["Location"]
                    ...
                    tik_get = (
                        await client.get(
                            "http://api.tiktokv.com/aweme/v1/multi/aweme/detail/?aweme_ids=["
                            + tik_info[0]
                        )
                    ).json()
                    ...
                    await self.inline.form(
                        message=message,
                        text=hlink(
                            tik_get["aweme_details"][0]["share_info"]["share_title"],
                            tik_get["aweme_details"][0]["share_info"]["share_url"],
                        ),
                        reply_markup=[
                            [
                                {
                                    "text": "Without watermark",
                                    "callback": self.download__callback,
                                    "args": (
                                        tik_get["aweme_details"][0]["video"][
                                            "play_addr"
                                        ]["url_list"][0],
                                        message.to_id,
                                    ),
                        ...
                        photo=tik_get["aweme_details"][0]["video"]["origin_cover"][
                            "url_list"
                        ][0],
                    )
                else:
