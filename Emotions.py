...
class EmotionsMod(loader.Module):
    ...
    strings = {
        ...
        "on": (
            "<emoji document_id=5373230475022179039>🥺</emoji> Emotions успешно"
            " активирован в этом чате."
        ),
        "off": (
            "<emoji document_id=5373230475022179039>🥺</emoji> Emotions успешно"
            " деактивирован в этом чате"
        ),
        "ok": (
            "<emoji document_id=5188315103384050849>☑️</emoji>Эмоция успешно добавлена"
        ),
        "list": (
            "<emoji document_id=5373230475022179039>🥺</emoji> Доступные"
            " эмоции:\n\n{}\n\n<emoji document_id=5467928559664242360>❗️</emoji> Для"
            " добавления своих эмоций введите команду:\n    <code>.emo</code>"
            " <символ/слово> <эмоция>"
        ),
    }
    ...
    async def emoclearcmd(self, message: Message):
        ...
        if args == "y":
            await self.allmodules.commands["e"](
                await utils.answer(
                    message, f"{self.get_prefix()}e db.pop('EmotionsMod')"
                )
            )
            await utils.answer(
                message,
                (
                    "<emoji document_id=5370842086658546991>☠️</emoji> <b>Список эмоций"
                    " успешно сброшен до зоводских настроек\nПожалуйста, загрузите"
                    " модуль еще раз.</b>"
                ),
            )
        else:
            await utils.answer(
                message,
                (
                    "<emoji document_id=5370842086658546991>☠️</emoji> <b>Вы не"
                    " подтвердили удаление!</b>"
                ),
            )
        ...
        await utils.answer(
            ...
            self.strings("list").format(
                "\n".join(
                    [
                        f" ▪️ {simvol} - {emotion}"
                        for simvol, emotion in self.emo.items()
                    ]
                )
            ),
        )
        ...
        try:
            ...
            if not args or simvol not in self.emo:
                await utils.answer(
                    message,
                    (
                        "<emoji document_id=5467928559664242360>❗️</emoji> Вы ввели"
    ...
    async def watcher(self, message: Message):
        ...
        await utils.answer(
            ...
            (
                ...
                f" <b>{utils.escape_html(msg)}</b> {emoji}"
            ),
        )
