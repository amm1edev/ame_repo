    ...
    async def rt(self, message: Message):
        ...
        if not args:
            await utils.answer(
                message,
                (
                    "<emoji document_id=5219806684066618617>🫠</emoji> <b>Подбираем"
                    " трек...</b>"
                ),
            )
            music = random.choice(
        ...
        if args == "m":
            ...
            mem = random.choice(
                await self.client.get_messages(
                    amems, limit=100, reply_to=utils.get_topic(message)
                )
            )
        ...
        if args == "my":
            await utils.answer(
                message,
                (
                    "<emoji document_id=5219806684066618617>🫠</emoji> <b>Подбираем трек"
                    " с вашего плейлиста...</b>"
                ),
            )
            my = random.choice(
                [
