...
class AntiMatMod(loader.Module):
    ...
    strings = {
        ...
        "list_txt": (
            "Здесь вы можете добавить свои маты.\np.s.: добавляйте по одному мату"
        ),
        "added": (
            "<b><emoji document_id=5030749344752468962>➕</emoji> Чат успешно добавлен в"
            " антимат систему</b>"
        ),
        "uadded": (
            "<b><emoji document_id=5033287275287413303>🗑</emoji> Чат успешно удален из"
            " системы антимат</b>"
        ),
    }
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                "list",
                (
                    "хер, хрен, хуй, пизда, бля, пох, еблан, еба, шлюха, сука, уебан,"
                    " пздц, пиздец, пиздос, хую, долбоеб, пидор, гандон, хуя"
                ),
        ...
        if antimat == True:
            if cid not in self.chats:
                for mat in mats:
                    ...
                    if m != -1:
                        await message.edit(
                            "<emoji document_id=5213285132709929474>🤬</emoji> <b>Не"
                            " матерись!</b>"
                        )
            else:
                for mat in mats:
