...
# ---------------------------------------------------------------------------------
# Name: kzdQuotes
# Description: KzdQuote a message
# Author: KeyZenD
# Commands:
# .qq | .qqset
# ---------------------------------------------------------------------------------
...
class kzdQuotesMod(loader.Module):
    ...
    async def qqcmd(self, message):
        ...
        name = (
            "Deleted Account"
            if user and sender.deleted
            else telethon.utils.get_display_name(sender)
        )
    ...
    if reply and reply.media:
        ...
        try:
            ...
            try:
                dice = (
                    True
                    if type(reply.media) == telethon.tl.types.MessageMediaDice
                    else False
                )
            except AttributeError:
                pass
        if dice:
            ...
            if reply.media.emoticon == "🎲":
                ...
                if dice_text == 1:
                    ...
                    return "{} {}".format(
                        reply.media.emoticon, kzdQuotesMod.strings["dart_thrown"]
                    )
                dice_type = kzdQuotesMod.strings["diceType_dart"]
                return "{} {}: {}".format(reply.media.emoticon, dice_type, dice_text)
            elif reply.media.emoticon == "🏀":
                return "{} {}".format(
                    reply.media.emoticon, kzdQuotesMod.strings["ball_thrown"]
                )
            else:
                return "Unsupported dice type ({}): {}".format(
                    reply.media.emoticon, reply.media.value
                )
        elif reply.poll:
            try:
                ...
                if reply.file.mime_type == "application/x-tgsticker":
                    ...
                    try:
                        ...
                        try:
                            ...
                            emoji = ""
                    caption = (
                        "{} {}".format(emoji, kzdQuotesMod.strings["mediaType_sticker"])
                        if emoji != ""
                        else kzdQuotesMod.strings["mediaType_sticker"]
                    )
                    return caption
                else:
                    if reply.sticker:
                        ...
                        caption = (
                            "{} {}".format(
                                emoji, kzdQuotesMod.strings["mediaType_sticker"]
                            )
                            if emoji != ""
                            else kzdQuotesMod.strings["mediaType_sticker"]
                        )
    ...
    if reply and reply.media:
        if reply.photo:
            ...
            if (
                reply.gif
                or reply.video
                or reply.file.mime_type == "application/x-tgsticker"
            ):
