...
# ---------------------------------------------------------------------------------
# Name: dialogs_manager
# Author: Den4ikSuperOstryyPer4ik
# Commands:
# .dialogsinfo | .dialog_clear | .dclear | .all_dialogs_clear | .leave_chats
# ---------------------------------------------------------------------------------
...
class DialogsManagerMod(loader.Module):
    ...
    strings = {
        ...
        "dialogs_info": (
            ...
            " All users PM ➪ {}\n<emoji document_id=6037355667365300960>👥</emoji> All"
            ...
            " document_id=5805441297535733440>🚫</emoji> Scam ➪  {}\n  {} Premium ➪ "
            " {}\n  <emoji document_id=5850654130497916523>✅️</emoji> Verified ➪  {}\n "
            " <emoji document_id=6050677771154231040>🗑</emoji> Deleted ➪  {}\n\n<emoji"
            " document_id=6037355667365300960>👥</emoji> Chats:\n\n  <emoji"
            " document_id=6323524880121726602>✅</emoji> Megagroups ➪  {}\n\n  <emoji"
            " document_id=6323380805443782200>☑️</emoji> Gigagroups ➪  {}\n\n  <emoji"
            " document_id=5805597488316419570>🚫</emoji> Fake ➪  {}\n  <emoji"
            " document_id=5805441297535733440>🚫</emoji> Scam ➪  {}\n  <emoji"
            " document_id=5850654130497916523>✅️</emoji> Verified ➪  {}\n\n<emoji"
            " document_id=5764623873974734153>📢</emoji> Channels:\n  <emoji"
            " document_id=5805597488316419570>🚫</emoji> Fake ➪  {}\n  <emoji"
            " document_id=5805441297535733440>🚫</emoji> Scam ➪  {}\n  <emoji"
            " document_id=5850654130497916523>✅️</emoji> Verified ➪  {}\n\n<emoji"
            " document_id=6037190220930092878>🤖</emoji> Bots(PM):\n  <emoji"
            " document_id=5805597488316419570>🚫</emoji> Fake ➪  {}\n  <emoji"
            " document_id=5805441297535733440>🚫</emoji> Scam ➪  {}\n<emoji"
            " document_id=5850654130497916523>✅️</emoji> Verified ➪  {}"
        ),
    ...
    @loader.command(ru_doc="<id or @username> ➪  удалить чат(диалог) с юзером")
    ...
    @loader.command(ru_doc="➪  алиас для команды 'dialogs_clear'")
    ...
    async def clear_dialogs(self, message):
        msg = await answer(
            message, "<b>Пожалуйста подождите, выполняется очистка...</b>"
        )
        args = get_args_raw(message)
        deleted_dialogs = 0
    ...
    @loader.command(
        ru_doc=(
            "@chat_username1, @chat_username2, ... ➪  покинуть чаты, с определенными"
            " @username"
        )
    )
    async def leave_chats(self, message: Message):
        ...
        if not args:
            ...
            return
        msg = await answer(
            message, "<b>Пожалуйста подождите, процесс выполняется...</b>"
        )
        dd = args.split(", ")
        leave_chatsss = 0
        for id in dd:
