...
class RandomUserMod(loader.Module):
    ...
    strings = {
        ...
        "inline-text": (
            "<b>👥 Click on the button to select a random user in the chat!</b>"
        ),
        ...
        "text-adm": (
            "<b>🎩 Was assigned to the administrator\n👨🏻‍💻 Prefix(rank): «{}»</b>"
        ),
        "mute-btn": "🤫 Mute",
        "unmute-btn": "😊 UnMute",
        "ban-btn": "👔 Ban",
        ...
        "invite-error-3": (
            "The user is kicked out of the chat, contact the administrators."
        ),
        "invite-error-4": "The bot is blocked in the chat, contact the administrators.",
        "invite-error-5": (
            "The user is blocked in the chat, contact the administrators."
        ),
    ...
    strings_ru = {
        "_cls_doc": (
            "Выбрать случайного пользователя в чате\nАвторы: @AstroModules &"
            " @smeowcodes"
        ),
        "inline-text": (
            "<b>👥 Нажмите на кнопку, чтобы выбрать случайного пользователя в чате:</b>"
        ),
        ...
        "invite-error-1": (
            "Настройки приватности пользователя не позволяют пригласить его."
        ),
        ...
        "invite-error-5": (
            "Пользователь заблокирован в чате, обратитесь к администраторам."
        ),
    ...
    @loader.command(
        ru_doc=(
            "--> выбрать случайного пользователя в чате | inline-меню с призовыми"
            " кнопками"
        )
    )
    async def irandusercmd(self, message: Message):
        ...
        self.users = [
            p.id async for p in self.client.iter_participants(message.peer_id)
        ]
    ...
    async def rand_user_inline(self, call: InlineCall):
        ...
        if not self.user.username:
            ...
            self.link = (
                f"<a href='https://t.me/{self.user.username}'>{self.user.first_name}</a>"
            )
            emoji_list2 = list(
                grapheme.graphemes("👤👶👧🧒👦👩🧑👨👩‍🦱🧑‍🦱👨‍🦱👩‍🦰🧑‍🦰👨‍🦰👱‍♀👱👱‍♂👩‍🦳🧑‍🦳👨‍🦳👵🧓👴")
            )
            emoji2 = r.choice(emoji_list2)
            self.id_or_username = self.strings("id+username").format(
                emoji2, self.user.username, self.user.id
            )
        ...
        await call.edit(
            ...
            reply_markup=[
                ...
                [
                    {"text": self.strings("ban-btn"), "callback": self.ban_user},
                    {"text": self.strings("kick-btn"), "callback": self.kick_user},
                    {"text": self.strings("mute-btn"), "callback": self.mute_user},
                ],
                [
                    {
    ...
    async def add_to_admins_user(self, call: InlineCall):
        ...
        try:
            ...
            return await call.edit(
                text=self.strings("user-...").format(
                    emoji, self.link, self.id_or_username, text
                ),
                reply_markup=[
                    [
                        {
    ...
    async def mute_user(self, call: InlineCall):
        ...
        try:
            ...
            await self.client(EditBannedRequest(self.chat_id, self.user.id, tm))
            await call.answer(text, show_alert=True)
            return await call.edit(
                text=self.strings("user-...").format(
                    emoji, self.link, self.id_or_username, text
    ...
    async def unmute_user(self, call: InlineCall):
        ...
        await self.client(EditBannedRequest(self.chat_id, self.user.id, UNMUTE_RIGHTS))
        await call.answer(text, show_alert=True)
        await call.edit(
    ...
    async def ban_user(self, call: InlineCall):
        ...
        try:
            ...
            await call.edit(
                ...
                reply_markup=[
                    [{"text": self.strings("unban-btn"), "callback": self.unban_user}],
                    [
                        {
                            "text": self.strings("rand-user-2-btn"),
                            "callback": self.rand_user_inline,
    ...
    async def kick_user(self, call: InlineCall):
        ...
        return await call.edit(
            ...
            reply_markup=[
                [{"text": self.strings("invite-btn"), "callback": self.invite_user}],
                [
                    {
                        "text": self.strings("rand-user-2-btn"),
                        "callback": self.rand_user_inline,
