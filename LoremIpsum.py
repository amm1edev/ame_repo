...
# ---------------------------------------------------------------------------------
# Name: LoremIpsum
# Description: Lorem Ipsum generation
# Author: KeyZenD
# Commands:
# .loremipsum
# ---------------------------------------------------------------------------------
    ...
    async def loremipsumcmd(self, message):
        ...
        if args:
            ...
            if len(args) == 2:
                ...
                length = (
                    s
                    if lenght in [s[: i + 1] for i in range(len(s))]
                    else (
                        l
                        if lenght in [l[: i + 1] for i in range(len(l))]
                        else v if lenght in [v[: i + 1] for i in range(len(v))] else m
                    )
                )  # сижу ахуел
            if len(args) >= 3:
                as_file = True
